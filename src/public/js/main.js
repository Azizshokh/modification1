...
// Used by both Member and Product status selects.
...
// Colors all user status selects on load and sends an Axios POST on change.
...
/* ============================================================
   PRODUCT — Table filter, status colors, form toggle,
             measurement fields, validation, image upload
   ============================================================ */
...
// Colors all product status selects on page load and sends an Axios POST on change.
// Reverts the select to its previous value if the request fails.
function initProductStatusColors() {
  document.querySelectorAll('.status-select[data-id]:not([data-type="user"])').forEach((select) => {
    if (select.dataset.productStatusBound === '1') {
      return;
    }
...
function initUserStatusColors() {
  document.querySelectorAll('.status-select[data-type="user"]').forEach((select) => {
    if (select.dataset.userStatusBound === '1') {
      return;
    }
